...
describe('Native Protocol v2 schema authority', () => {
  ...
  test('versions Android link-control schema additions with ABI v5 while retaining protocol v2 records', () => {
    const schema = JSON.parse(fs.readFileSync(schemaPath, 'utf8'))
    const manifest = JSON.parse(fs.readFileSync(path.join(path.dirname(schemaPath), schema.abiManifest), 'utf8'))
    const cpp = read('native/protocol/generated/NativeProtocolV2Schema.hpp')
    const kotlin = read(
    ...
    expect(swift).toContain('nativeProtocolABIVersion: UInt32 = 5')
    ...
    expect(typescript).toContain('export const NATIVE_PROTOCOL_ABI_VERSION = 5')
