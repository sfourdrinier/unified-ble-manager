...
describe('PR10 Expo documentation', () => {
  test('documents Expo v2 as the published RC4 installation path', () => {
    ...
    expect(documents).toContain(JSON.parse(read('package.json')).version)
    expect(documents).toContain('4.0.0-rc.4')
    expect(documents).toContain(`pnpm add unified-ble-manager@${JSON.parse(read('package.json')).version}`)
