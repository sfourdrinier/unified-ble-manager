...
describe('injectable random source', () => {
  ...
  test('does not require a computed crypto module', () => {
    const source = require('fs').readFileSync(require('path').join(__dirname, '../src/public/host-identity.ts'), 'utf8')
    expect(source).not.toMatch(/tryNodeRandomBytes/)
    expect(source).not.toMatch(/Reflect\.apply\(require/)
