...
describe('consumer documentation matches the published package', () => {
  ...
  test('RC4 candidate documentation preserves release, evidence, and deferral boundaries', () => {
    ...
    expect(readme).toContain('`4.0.0-rc.5` is published from exact `main`')
    ...
    expect(platforms).toContain(
      '`unified-ble-manager@4.0.0-rc.5` is the published **release-candidate package/API** for the 4.x contract; it is immutable and stable `4.0.0` remains reserved for the post-PR12 release gate'
    )
    expect(platforms).toContain(
