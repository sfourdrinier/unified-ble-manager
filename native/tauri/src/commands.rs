...
pub async fn invoke<R: Runtime>(
    ...
    for permission in security_permissions_for(&request) {
...
fn security_permissions_for(request: &IpcValue) -> Vec<SecurityPermission> {
    let IpcValue::Object(fields) = request else {
        return Vec::new();
    };
    ...
    let Some(IpcValue::Object(envelope)) = fields.get("envelope") else {
        return Vec::new();
    };
    let Some(IpcValue::String(command)) = envelope.get("command") else {
        return Vec::new();
    };
    let Some(permission) = security_permission_for_command(command) else {
        return Vec::new();
    };
    let mut permissions = vec![permission];
    if permission == SecurityPermission::Pair
        && pairing_uses_custom_ceremony(envelope.get("payload"))
    {
        permissions.push(SecurityPermission::CustomCeremony);
    }
    permissions
}
...
fn pairing_uses_custom_ceremony(payload: Option<&IpcValue>) -> bool {
    let Some(IpcValue::Object(payload)) = payload else {
        return false;
    };
    match payload.get("ceremony") {
        Some(IpcValue::String(ceremony)) => ceremony != "system",
        Some(IpcValue::Object(_)) => true,
        _ => false,
    }
}
...
mod tests {
    ...
    use super::security_permissions_for;
    use crate::{security_scope::SecurityPermission, IpcValue};

    fn route(command: &str) -> IpcValue {
        route_with_payload(command, None)
    }

    fn route_with_payload(command: &str, payload: Option<IpcValue>) -> IpcValue {
        let mut envelope =
            BTreeMap::from([("command".to_owned(), IpcValue::String(command.to_owned()))]);
        if let Some(payload) = payload {
            envelope.insert("payload".to_owned(), payload);
        }
        IpcValue::Object(BTreeMap::from([
            ("kind".to_owned(), IpcValue::String("route".to_owned())),
            ("envelope".to_owned(), IpcValue::Object(envelope)),
        ]))
    }
    ...
    fn security_commands_map_to_independent_permission_atoms() {
        ...
        assert_eq!(
            security_permissions_for(&route("security.pair")),
            vec![SecurityPermission::Pair]
        );
        assert_eq!(
            security_permissions_for(&route("security.cancel-pairing")),
            vec![SecurityPermission::CancelPairing]
        );
        assert_eq!(
            security_permissions_for(&route("security.unpair")),
            vec![SecurityPermission::Unpair]
        );
        assert_eq!(
            security_permissions_for(&route("security.custom-ceremony")),
            vec![SecurityPermission::CustomCeremony]
        );
    }
    ...
    fn unknown_commands_and_renderer_scope_fields_do_not_grant_security_permission() {
        assert_eq!(
            security_permissions_for(&route("security.wildcard")),
            Vec::new()
        );
        assert_eq!(
            security_permissions_for(&IpcValue::Object(BTreeMap::from([
                ("kind".to_owned(), IpcValue::String("route".to_owned())),
                (
    ...
    #[test]
    fn custom_pairing_requires_pair_and_custom_ceremony_permissions() {
        let custom_payload = IpcValue::Object(BTreeMap::from([(
            "ceremony".to_owned(),
            IpcValue::String("custom".to_owned()),
        )]));
        assert_eq!(
            security_permissions_for(&route_with_payload("security.pair", Some(custom_payload))),
            vec![SecurityPermission::Pair, SecurityPermission::CustomCeremony]
        );

        let agent_payload = IpcValue::Object(BTreeMap::from([(
            "ceremony".to_owned(),
            IpcValue::Object(BTreeMap::from([(
                "kind".to_owned(),
                IpcValue::String("agent".to_owned()),
            )])),
        )]));
        assert_eq!(
            security_permissions_for(&route_with_payload("security.pair", Some(agent_payload))),
            vec![SecurityPermission::Pair, SecurityPermission::CustomCeremony]
        );
    }
}
