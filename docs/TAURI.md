...
**IPC protocol version.** The webview and the plugin speak IPC protocol 3 and each offers exactly that version. Version 3 is the wire described on this page: relative `budgetMs` deadlines, `commit` on every error, `delivery` on subscriptions, and forwarded connection-lifecycle events. A pair where one side is older (protocol 2, the 5.0.0-rc.0 wire) is refused at bootstrap with `protocol.incompatible` before any operation runs, whichever side is older; the npm package and the crate must be upgraded together. `TAURI_PLUGIN_COMPATIBILITY.ipcProtocol` reports the version this package requires.
...
A notification stream ends with the core's own reason: `connection-lost`, `service-changed`, `overflow`, or `source-failed` when the core closed a stream that is still mapped (with `operation.reset` when an adapter loss ended it). A scan the OS or an adapter loss ended without a stop request ends `source-failed` with the core's own words (`scan.start-failed`, platform detail), or `closed` when the OS stopped it without an error; nothing is left to stop. Each value carries the `delivery` the radio reported for the subscription (`notification`, `indication`, or `unknown` when the platform does not say); the subscribe response reports it too.
...
When the adapter powers off, resets, becomes unsupported or unauthorized, is removed, or its daemon restarts, the core tears everything down: in-flight operations fail `operation.reset`, the scan and every notification stream end `source-failed`, every link ends with `adapter-loss`, and the attachment moves to a new backend and adapter generation. The webview's attachment is then the previous one: every later request on it fails `backend.reset` (recovery: recreate the manager) before any native call, except releases (`scan.stop`, `gatt.unsubscribe`, `gatt.database.release`, `connection.disconnect`, `connection.events.unsubscribe`, `operation.cancel`) and `manager.destroy()`, which still settle what the webview holds. A new manager attaches to the new generation. No adapter-state event is pushed; read `adapter.state` for the current facts.
