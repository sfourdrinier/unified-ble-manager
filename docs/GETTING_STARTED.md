...
This source targets `4.0.9`; verify the published version in the npm registry.
...
After the npm registry lists `4.0.9`, install that exact version and keep it in
your lockfile while validating the native build:

```sh
pnpm add unified-ble-manager@4.0.9
```
