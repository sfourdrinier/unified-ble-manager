...
`14850ffa7274fae08742788517a6612df63ea871`; documentation-only descendants
...
Remaining release gates at this checkpoint: final local artifact/native/performance gates for `14850ff`, commit the pending docs/API slice, push and hosted rerun at that exact final source tree, final exact-SHA adversarial review, the required external review rounds, merge PR41, then proceed to the plan’s RC3 checkpoint. Physical-radio qualification and the opt-in Apple compile lane remain separately labeled; RC2 remains immutable.
