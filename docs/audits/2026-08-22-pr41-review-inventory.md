...
`4ec3298df86a77dc38f222c2cbd7cd71c08963f2`; later tests, CI/scripts, and
documentation commits preserve this runtime source tree.
The full package gate is 142 suites / 1,343 tests. Focused settlement,
...
| F-30 | Final lifecycle review | BlueZ cleanup and scan/database/shared-lease ownership could be lost after native cleanup failure, while WinRT disconnect/retirement could remain unbounded. | Confirmed and fixed in `24c7636`/`1dc0e6b`; cleanup retains ownership, bounds native waits, preserves retry state, and covers delayed settlement. |
...
| F-32 | Final lifecycle review | CoreBluetooth stop-notify, stop-scan, disconnect, adapter-loss, and late-connect native callbacks needed one green memoized timeout transition that preserves in-flight ownership without regressing baseline coverage. | Confirmed and fixed in `4ec3298`; timeout/retry transitions retain original native promises, avoid duplicate calls, preserve counters, and 52 focused CoreBluetooth lifecycle tests pass. |
...
Remaining release gates at this checkpoint: push the final source tree at `4ec3298`, hosted Node 22/24 generic, G6A, and Expo/Tauri packed proofs at that exact source SHA, final exact-SHA adversarial review, the required external review rounds, merge PR41, then proceed to the plan’s RC3 checkpoint. Physical-radio qualification and the opt-in Apple compile lane remain separately labeled; RC2 remains immutable.
