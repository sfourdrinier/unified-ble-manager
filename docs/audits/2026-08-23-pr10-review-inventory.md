...
| F10-007 | Android foreground-service/CDM and native restoration execution are not yet implemented by the current partial diff. | Android now has a metadata-gated `BlePlxForegroundService`, acknowledged promotion, restart-policy handling, TurboModule acquire/release methods, and ref-counted leases; Companion Device Manager ceremony and full restoration example remain absent. | Partially addressed; CDM/example diagnostics remain PR10D work. | Android Gradle/unit gate, lifecycle evidence, and explicit support limitations. |
...
Expo example screens/diagnostics and Companion Device Manager ceremony, an
Apple full-module/Expo host build (the local Swift harness is not that proof),
fresh adversarial review of the post-fix SHA, GitHub Actions, and the required
two-round PR review cycle. Local deterministic package, artifact, packed,
performance, Android, and protocol gates listed above are already passing.
