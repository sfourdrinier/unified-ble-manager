...
tip `56f5492`; the runtime hardening slices are committed separately as
`4224c0a`/`b7aa1f8` (CoreBluetooth), `1fd13f2`/`56f5492` (BlueZ), and `4297f7b` (WinRT). The readiness ingress, core
...
| Review/release | PR #41 is open; first Copilot/Codex findings and current Codex threads are dispositioned through `56f5492`; full CoreBluetooth/BlueZ/WinRT focused sets are green, while hosted rerun, external review, merge, and RC3 remain pending | Blocking; RC2 remains immutable |
