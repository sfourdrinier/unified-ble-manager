...
Current PR7 implementation checkpoint: `feat/4.0-security-pairing` / `3c68577` (implementation tip); documentation-only descendants from this commit
...
candidate passed (123 suites, 1,168 tests), lint/typecheck, clippy, native protocol,
...
| Package/prepack/deterministic packed consumer | Proven locally | `pnpm test:package` passed 123 suites / 1,168 tests, including prepack and G6A packed-consumer coverage. |
...
| Hosted CI and release qualification | Awaiting final exact code tip `3c68577` | Prior hosted PR40 run `32555035697` is green across JS Node22/24/macOS/Windows, Tauri macOS/Linux/Windows, Expo CNG Android, and Classic RN Android; the final code-tip run must repeat those gates. Apple compile checks are skipped by the workflow’s opt-in policy. |
