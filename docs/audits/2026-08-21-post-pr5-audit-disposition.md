...
`f07ac04`. The readiness ingress, core
...
| Local package gate | `pnpm test:package`: 140 suites / 1,290 tests; `pnpm test:plugin`: 36 tests; docs/API, native protocol, artifact, smell, lint/typecheck, and diff checks included | Green locally |
...
| Review/release | PR #41 is open; first Copilot/Codex findings and current Codex threads are dispositioned through `f07ac04`; hosted CI rerun including G6A, final exact-SHA review, merge, and RC3 remain pending | Blocking; RC2 remains immutable |
