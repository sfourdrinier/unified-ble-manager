...
`unified-ble-manager@4.0.0-rc.3` is the prepared release-candidate package/API. The RC3 tag workflow publishes this candidate after the exact-main release gates pass; stable `4.0.0` is reserved for the
post-PR12 release gate. The RC3 package does not rewrite the evidence inventory: a backend remains at the support level
