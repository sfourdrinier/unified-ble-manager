...
An adapter loss is a power-off, resetting, unsupported, revoked authorization, a removed adapter, or a restarted bluetoothd. It tears down everything live. In-flight operations settle `operation.reset`. Scans and subscriptions end `source-failed`. Links are released: CoreBluetooth and WinRT emit `connection-state-changed` with reason `adapter`, while BlueZ invalidates them without an event. The backend generation advances, so every handle and peer id from before the loss is stale. CoreBluetooth and BlueZ then emit `backend-restarted`. Each OS keeps the sequence its 4.x backend had.
