...
| --- | --- | --- | --- |
| scan session | starting, active, stopping, stopped, failed | starting → active or stopping; active → stopping; stop/abort/deadline → stopping → stopped; host/source terminals leave `active` without `stop()` (`failed` for `source-failed`/`connection-lost`, `stopped` for ordinary close) and that event is ended delivery, not physical cleanup; reset/restart/destroy → failed | stopped includes one terminal cause; failed rejects reuse. |
