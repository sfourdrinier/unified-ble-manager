...
history. `4.0.28` is immutable tagged history. This branch prepares `5.0.0-rc.3`.
...
`x86_64-linux-android` for the pinned toolchain. Do not move any failed tag or
publish any failed version from a workstation.

Integrate the `5.0.0` release branch into `main`, then release
`v5.0.0-rc.3` only from that exact current `main` commit after canonical CI
succeeds. This matches the publish workflow's immutable main-source gate.
Verify `package.json` is `5.0.0-rc.3`, the worktree is clean, and release-note
extraction finds `## [5.0.0-rc.3]`. Push a new
annotated `v5.0.0-rc.3` tag with the GitHub Release marked prerelease and the
...
- npm `next` resolves to `5.0.0-rc.3`, while `latest` remains on the 4.0 stable
...
- a clean consumer, in a directory outside this repository, can install
  `unified-ble-manager@5.0.0-rc.3` explicitly and import the documented host
...
Prereleases such as `5.0.0-rc.3` use normal SemVer suffixes. They publish to
