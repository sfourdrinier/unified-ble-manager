...
history. `4.0.28` is immutable tagged history. This branch prepares `5.0.0-rc.2`.
...
`tools` SDK package. The corrected workflow uses Xvfb and explicitly installs
only `platform-tools`. Do not move either failed tag or publish either version
from a workstation.

Integrate the `5.0.0` release branch into `main`, then release
`v5.0.0-rc.2` only from that exact current `main` commit after canonical CI
succeeds. This matches the publish workflow's immutable main-source gate.
Verify `package.json` is `5.0.0-rc.2`, the worktree is clean, and release-note
extraction finds `## [5.0.0-rc.2]`. Push a new
annotated `v5.0.0-rc.2` tag with the GitHub Release marked prerelease and the
...
- npm `next` resolves to `5.0.0-rc.2`, while `latest` remains on the 4.0 stable
...
- a clean consumer, in a directory outside this repository, can install
  `unified-ble-manager@5.0.0-rc.2` explicitly and import the documented host
...
Prereleases such as `5.0.0-rc.2` use normal SemVer suffixes. They publish to
