...
export function assertPeerReference(reference: unknown, operation: string): asserts reference is PeerReference {
  if (typeof reference !== 'object' || reference === null || Array.isArray(reference)) {
    throw contractError('peer.reference-invalid', 'connection', operation)
  }
  const keys = Object.keys(reference)
  if (keys.length !== 4 || keys.some(key => !['version', 'backendId', 'scope', 'opaqueId'].includes(key))) {
    throw contractError('peer.reference-invalid', 'connection', operation)
  }
...
export function isPeerReference(value: unknown): value is PeerReference {
  ...
  if (keys.length !== 4 || keys.some(key => !['version', 'backendId', 'scope', 'opaqueId'].includes(key))) return false
  const version = Reflect.get(value, 'version')
  const backendId = Reflect.get(value, 'backendId')
  const scope = Reflect.get(value, 'scope')
  const opaqueId = Reflect.get(value, 'opaqueId')
  return (
    version === 1 &&
    typeof backendId === 'string' &&
    backendId.length > 0 &&
    typeof scope === 'string' &&
    (scope === 'application' || scope === 'origin' || scope === 'system') &&
    typeof opaqueId === 'string' &&
    opaqueId.length > 0
  )
}

export function snapshotPeerReference(reference: unknown, operation: string): PeerReference {
