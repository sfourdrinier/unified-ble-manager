...
impl BluetoothSession {
    ...
    /// [`BluetoothSession::new`], on the D-Bus *session* bus instead of the
    /// system bus — for a BlueZ service exported on the session bus (mock
    /// BlueZ, sandboxed setups), which the legacy UBM BlueZ backend
    /// supported as `busKind: 'session'`.
    pub async fn new_session_bus()
    -> Result<(impl Future<Output = Result<(), SpawnError>>, Self), BluetoothError> {
        let (dbus_resource, connection) = dbus_tokio::connection::new_session_sync()?;
        connection.set_signal_match_mode(true);
        let dbus_handle = tokio::spawn(async {
            let err = dbus_resource.await;
            Err(SpawnError::DbusConnectionLost(err))
    ...
    async fn await_service_discovery(&self, device_id: &DeviceId) -> Result<(), BluetoothError> {
        ...
        let events = self.device_event_stream(device_id).await?;
        if self
        ...
        timeout(
            ...
            service_discovery_outcome(device_id, events),
        )
        .await
...
/// UBM patch (vendor/btleplug/UBM_PATCHES.md #19): the first answer the
/// device's events give to a pending service discovery. A disconnect ends
/// the wait at once, as the legacy BlueZ backend failed its pending work on
/// disconnect; upstream ignored it and waited for the timeout.
async fn service_discovery_outcome(
    device_id: &DeviceId,
    mut events: impl Stream<Item = BluetoothEvent> + Unpin,
) -> Result<(), BluetoothError> {
    while let Some(event) = events.next().await {
        let BluetoothEvent::Device { id, event } = event else {
            continue;
        };
        if &id != device_id {
            continue;
        ...
        match event {
            DeviceEvent::ServicesResolved => return Ok(()),
            DeviceEvent::Connected { connected: false } => {
                return Err(BluetoothError::DisconnectedDuringServiceDiscovery);
            }
            _ => {}
        }
    }
...
mod ubm_tests {
    ...
    fn device_event(path: &str, event: DeviceEvent) -> BluetoothEvent {
    ...
    #[tokio::test]
    async fn a_disconnect_ends_a_pending_service_discovery() {
        let device = DeviceId::new("/org/bluez/hci0/dev_11_22_33_44_55_66");
        let events = stream::iter(vec![
            device_event(
                "/org/bluez/hci0/dev_66_55_44_33_22_11",
        ...
        assert!(matches!(
            service_discovery_outcome(&device, events).await,
            Err(BluetoothError::DisconnectedDuringServiceDiscovery)
        ));
    }

    #[tokio::test]
    async fn resolved_services_end_the_discovery() {
        let device = DeviceId::new("/org/bluez/hci0/dev_11_22_33_44_55_66");
        let events = stream::iter(vec![
            device_event(
                "/org/bluez/hci0/dev_11_22_33_44_55_66",
