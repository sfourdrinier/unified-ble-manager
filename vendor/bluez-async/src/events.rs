...
pub enum DeviceEvent {
    ...
    ServicesResolved,
    /// UBM patch (`vendor/btleplug/UBM_PATCHES.md` #18): one `Device1`
    /// `PropertiesChanged` signal, whatever it changed (`Name`, `Alias`,
    /// `TxPower`, `Appearance`, `AddressType`, `Class`, RSSI, data, …),
    /// raised once per signal after the specific events above. `properties`
    /// lists the changed and invalidated property names, sorted. Upstream
    /// raised nothing for most of them (a name-only change was invisible);
    /// the legacy BlueZ backend reported the device on every change.
    PropertiesChanged { properties: Vec<String> },
}
    ...
    fn properties_changed_to_events(
        ...
        match properties_changed.interface_name.as_ref() {
            ...
            ORG_BLUEZ_DEVICE1_NAME => {
                ...
                let mut properties: Vec<String> = changed_properties
                    .keys()
                    .cloned()
                    .chain(properties_changed.invalidated_properties.iter().cloned())
                    .collect();
                properties.sort();
                properties.dedup();
                if !properties.is_empty() {
                    events.push(BluetoothEvent::Device {
                        id,
                        event: DeviceEvent::PropertiesChanged { properties },
                    });
                }
            }
...
mod tests {
    ...
    fn device_rssi() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                // UBM patch #18: every Device1 change is also summarised.
                BluetoothEvent::Device {
                    id,
                    event: DeviceEvent::PropertiesChanged {
                        properties: vec!["RSSI".to_owned()]
                    }
                }
            ]
        )
        ...
        assert_eq!(
            ...
            vec![
                ...
                BluetoothEvent::Device {
                    id,
                    event: DeviceEvent::PropertiesChanged {
                        properties: vec!["ManufacturerData".to_owned()]
                    }
                }
            ]
        )
        ...
        assert_eq!(
            ...
            vec![
                ...
                BluetoothEvent::Device {
                    id,
                    event: DeviceEvent::PropertiesChanged {
                        properties: vec!["ServiceData".to_owned()]
                    }
                }
            ]
        )
    ...
    /// UBM patch #18: a name-only or alias-only change, and any other
    /// Device1 property change, raises one summary event (upstream raised
    /// none), with the changed and invalidated names sorted.
    #[test]
    fn device_name_alias_and_other_changes_are_reported() {
        let path = "/org/bluez/hci0/dev_11_22_33_44_55_66";
        let id = DeviceId::new(path);
        for (changes, invalidated, expected) in [
            (vec![("Name", "Polar H10")], vec![], vec!["Name"]),
            (vec![("Alias", "My strap")], vec![], vec!["Alias"]),
            (
                vec![("Name", "Polar"), ("Alias", "Polar")],
                vec!["TxPower"],
                vec!["Alias", "Name", "TxPower"],
            ),
        ] {
            let mut changed_properties: PropMap = HashMap::new();
            for (key, value) in changes {
                changed_properties.insert(key.to_string(), Variant(Box::new(value.to_string())));
            }
            let message = PropertiesPropertiesChanged {
                interface_name: "org.bluez.Device1".to_string(),
                changed_properties,
                invalidated_properties: invalidated.into_iter().map(str::to_owned).collect(),
            }
            .to_emit_message(&path.into());
            assert_eq!(
                BluetoothEvent::message_to_events(message),
                vec![BluetoothEvent::Device {
                    id: id.clone(),
                    event: DeviceEvent::PropertiesChanged {
                        properties: expected.into_iter().map(str::to_owned).collect()
                    }
                }]
            );
        }
