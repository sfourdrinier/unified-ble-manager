...
**RC3 candidate:** `4.0.0-rc.3` is prepared in this source tree. Publication to npm `latest` remains pending the tag-driven trusted-publishing workflow. Package SemVer and backend support labels are independent: each radio backend stays Experimental until artifact-bound physical-hardware validation says otherwise. See [`docs/PLATFORMS.md`](docs/PLATFORMS.md).
