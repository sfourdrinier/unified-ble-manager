...
- `BleDiagnosticTraceDocument :: { readonly format: "unified-ble-trace-v1"; readonly truncated: boolean; readonly records: readonly BleDiagnosticTraceRecord[] }`
- `BleDiagnosticTraceRecord :: { readonly ordinal: number; readonly time: number; readonly kind: "operation" | "resource" | "stream" | "attachment"; readonly event: string; readonly cause: string | null; readonly correlation: string | null; readonly redactedClient: boolean; readonly redactedPeer: boolean; readonly redactedPath: boolean; readonly redactedPayload: boolean }`
- `BleDiagnostics :: { snapshot(): BleDiagnosticsSnapshot; resourceCounters(): BleResourceCounters; startTrace(): { stop(): Promise<BleDiagnosticTraceDocument>; } }`
- `BleDiagnosticsSnapshot :: { readonly trace: BleDiagnosticTraceDocument; readonly resourceCounters: BleResourceCounters }`
...
- `BleRetryability :: "never" | "caller-decides"`
...
- `BuiltInFeatureId :: "discovery:continuous-scan" | "discovery:system-chooser" | "discovery:advertisement-watch" | "scan:platform-options" | "peer:resolve-reference" | "peer:address-targeting" | "peer:known" | "peer:system-connected" | "peer:bonded" | "peer:origin-authorized" | "peer:restored" | "connection:direct" | "connection:when-available" | "connection:rssi" | "connection:effective-mtu" | "connection:request-mtu" | "connection:priority" | "connection:parameters" | "connection:phy" | "connection:subrate" | "security:state" | "security:pair" | "security:cancel-pairing" | "security:unpair" | "security:custom-ceremony" | "security:pairing-generation" | "gatt:descriptors" | "gatt:indications" | "gatt:service-changed" | "gatt:maximum-write-length" | "gatt:long-write" | "gatt:reliable-write" | "gatt:write-without-response-readiness" | "gatt:high-throughput-acquire" | "background:apple-restoration" | "background:android-connected-device-service" | "background:desktop-maintain-connection" | "background:wake-on-appearance" | "background:native-resubscribe" | "background:headless-task" | "background:wake-notification" | "lifecycle:page-persistence"`
...
- `ConnectionSupervisor :: { readonly events: PublicBoundedAsyncStream<ConnectionSupervisorEvent<Session>>; readonly snapshot: ConnectionSupervisorSnapshot<Session>; start(): void; pause(reason?: string | undefined): Promise<void>; resume(): void; reconnectNow(): void; stop(): Promise<CleanupRecord> }`
...
- `CustomStreamBudget :: { readonly itemCapacity: number; readonly byteCapacity: number; readonly reservedControlCapacity?: number | undefined; readonly overflowPolicy?: PublicStreamOverflowPolicy | undefined }`
...
- `GattValueStream :: { readonly limits: PublicStreamLimits; readonly overflowPolicy: PublicStreamOverflowPolicy; close(): Promise<CleanupRecord>; __@asyncIterator@169(): PublicBoundedAsyncStreamIterator<GattValueEvent> }`
...
- `PairCancelResult :: { readonly outcome: "cancelled"; } | { readonly outcome: "not-pairing"; } | { readonly outcome: "paired"; } | { readonly outcome: "rejected"; readonly reason: string | null; }`
- `PairOptions :: { readonly transport?: "le" | "auto" | undefined; readonly protection?: "encrypted" | "authenticated" | "system-default" | undefined; readonly ceremony?: PairingAgent | "system" | undefined; readonly secureConnections?: "require" | "prefer" | "disallow" | undefined; readonly signal?: AbortSignal | undefined; readonly timeoutMs?: number | undefined }`
...
- `PublicBoundedAsyncStream :: { readonly limits: PublicStreamLimits; readonly overflowPolicy: PublicStreamOverflowPolicy; close(): Promise<CleanupRecord>; __@asyncIterator@169(): PublicBoundedAsyncStreamIterator<Value> }`
- `PublicBoundedAsyncStreamIterator :: { readonly return: () => Promise<IteratorResult<PublicStreamItem<Value>, undefined>>; __@asyncIterator@169(): PublicBoundedAsyncStreamIterator<Value>; next(...[value]: [] | [undefined]): Promise<IteratorResult<PublicStreamItem<Value>, undefined>>; throw?: ((e?: any) => Promise<IteratorResult<PublicStreamItem<Value>, undefined>>) | undefined }`
- `PublicPlatformErrorDetail :: { readonly domain: string; readonly code: string; readonly safeMessage: string; readonly metadata: PublicSerializableRecord }`
...
- `PublicSerializableValue :: string | number | boolean | PublicSerializableRecord | Uint8Array<ArrayBufferLike> | readonly PublicSerializableValue[] | null`
- `PublicStreamItem :: PublicStreamValue<Value> | PublicStreamOverflowNotice | PublicStreamTerminalNotice`
