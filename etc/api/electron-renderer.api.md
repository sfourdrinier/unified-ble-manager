...
- `ElectronRendererIpcTransport :: { invoke<Operation extends string>(request: IpcBleRequest<Attachment, Client, Operation>): Promise<IpcBleResponse<Attachment, Client>>; subscribe(listener: (event: IpcBleEvent) => void): () => void; subscribeEventHealth?: ((listener: (notice: IpcEventTransportHealthNotice) => void) => () => void) | undefined; acknowledge(rendererLease: RendererLeaseIdentity, eventId: string): Promise<IpcEventAcknowledgeResponse | IpcFailureResponse> }`
