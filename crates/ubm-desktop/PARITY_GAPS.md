...
shrinking of the matrix. Peripheral/server roles are required 5.0 outcomes
...
| `gatt:indications` | gatt.reads-descriptors-write-policy-and-dispatched-cancellation | Subscribed; delivery reported as kind unknown (`delivery-kind-unknown`) because the btleplug stream does not distinguish indications from notifications. |
...
| `connection:effective-mtu` | connection.rssi-and-att-mtu-capability-contract | Wiring `mtu()` into the maximum-write-length path (measured, not guessed). |
...
| `gatt:maximum-write-length` | gatt.maximum-write-length | Wiring measured MTU into the core maximum-write-length path end to end. |
...
35 required desktop rows: 6 btleplug-provides, 21 narrow-OS-adapter-needed,
